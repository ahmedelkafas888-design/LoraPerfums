...
const LoraAdminEngineModule = {
    ...
    bindEvents() {
        ...
        ["products", "coupons", "settings", "orders"].forEach(tab => {
    ...
    listenRealtimeStreams() {
        ...
        onSnapshot(collection(db, "orders"), (snapshot) => {
            ...
            snapshot.forEach(docSnap => {
                ...
                sortedDocs.push({ id: docSnap.id, data: o });
            });

            sortedDocs.sort((a, b) => (b.data.createdAt || 0) - (a.data.createdAt || 0));

            const badgeCounter = document.getElementById("adminPendingOrdersBadgeCounter");
            if (badgeCounter) {
                if (pendingCount > 0) {
            ...
            const txtCounter = document.getElementById("adminPendingOrdersDashboardStatusBadge");
            if (txtCounter) txtCounter.innerText = pendingCount;

            sortedDocs.forEach(item => {
                const o = item.data;
                const dateString = o.createdAt ? new Date(o.createdAt).toLocaleString() : 'N/A';
                let itemsListText = o.cartItems ? o.cartItems.map(i => `${i.nameEn || i.nameAr} (x${i.qty})`).join(", ") : "";

                const orderNode = document.createElement("div");
                orderNode.className = "bg-[#121212] border border-neutral-800 p-4 space-y-3 font-sans text-xs text-neutral-300 flex flex-col justify-between";
                orderNode.innerHTML = `
                    <div class="space-y-3">
                        <div class="flex justify-between items-start border-b border-neutral-800 pb-2">
                            <div>
                                <p class="text-[10px] text-neutral-500 font-mono">ID: ${item.id}</p>
                                <p class="text-white font-bold mt-1">${o.fullName || 'N/A'} (${o.phone || 'N/A'})</p>
                                <p class="text-neutral-400">${o.city || ''}, ${o.address || ''}</p>
                            </div>
                            <div class="text-right">
                                <span class="text-amber-500 font-bold font-mono text-sm">${o.totalPrice || 0} EGP</span>
                    ...
                    <div class="flex flex-col gap-2 pt-2 border-t border-neutral-800/60 mt-4">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] uppercase tracking-wider text-neutral-400">Status Matrix:</span>
                            <span class="uppercase text-[10px] tracking-widest px-2 py-0.5 font-bold ${
                                (o.status === 'pending' || o.status === 'new') ? 'bg-blue-900/40 text-blue-400 border border-blue-500/20' :
                                o.status === 'confirmed' ? 'bg-amber-900/40 text-amber-400 border border-amber-500/20' :
                                o.status === 'shipped' ? 'bg-purple-900/40 text-purple-400 border border-purple-500/20' :
                                o.status === 'delivered' ? 'bg-green-900/40 text-green-400 border border-green-500/20' :
                                'bg-red-900/40 text-red-400 border border-red-500/20'
                            }">${(o.status || 'PENDING').toUpperCase()}</span>
                        </div>
                        <div class="grid grid-cols-5 gap-1 text-[9px] font-mono">
                            <button data-status="pending" class="status-change-trigger p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 ${ (o.status === 'pending' || o.status === 'new') ? 'border-amber-500 text-amber-500' : ''}">Pend</button>
                            <button data-status="confirmed" class="status-change-trigger p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 ${o.status === 'confirmed' ? 'border-amber-500 text-amber-500' : ''}">Conf</button>
                            <button data-status="shipped" class="status-change-trigger p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 ${o.status === 'shipped' ? 'border-amber-500 text-amber-500' : ''}">Ship</button>
                            <button data-status="delivered" class="status-change-trigger p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 ${o.status === 'delivered' ? 'border-amber-500 text-amber-500' : ''}">Deliv</button>
                            <button data-status="cancelled" class="status-change-trigger p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 ${o.status === 'cancelled' ? 'border-amber-500 text-amber-500' : ''}">Cancl</button>
                        </div>
                    </div>
                `;

                orderNode.querySelectorAll(".status-change-trigger").forEach(btn => {
                    btn.addEventListener("click", () => this.updateOrderStatus(item.id, btn.getAttribute("data-status")));
                });
