    ...
    try {
        onSnapshot(doc(db, "settings", "config_ledger"), (snapshot) => {
            if (snapshot.exists()) {
                ...
                if (data && data.whatsappNumber) AppState.whatsappNumber = data.whatsappNumber;
            } else {
                setDoc(doc(db, "settings", "config_ledger"), { whatsappNumber: "01208711729" })
                .catch(e => console.error("Config ledger initializing catch silenced:", e));
            }
        }, (err) => {
            console.error("Config fallback channel engaged:", err);
        });
    } catch (e) {
        console.error("Settings stream registration skipped safely:", e);
    }
