...
const AppState = {
    ...
    telegramBotToken: "8835968708:AAG8SCI-_RDTbHJkJr2up7JzBzkhJXV9N9w", // تم التحديث للبوت الجديد
    telegramChatId: "1627760104", // تم التحديث لـ الـ Chat ID الجديد
    ...
    async logVisitor() {
        try {
            ...
            window.addEventListener("beforeunload", () => {
                navigator.sendBeacon(`https://lora-perfum.firebaseapp.com/kill-session?id=${activeSessionId}`);
            });
        } catch (e) {
...
const NotificationEngine = {
    async broadcastTelegramAlert(orderId, name, phone, city, address, orderedItems, totalPrice) {
        try {
            // صياغة قائمة المنتجات المرسلة في التلغرام
            let productsLine = "";
            orderedItems.forEach((item, index) => {
                const prodName = AppState.lang === 'en' ? item.nameEn : item.nameAr;
                productsLine += `\n   ${index + 1}. ${prodName} (${item.size}ml) x${item.qty} - [${item.price} EGP]`;
            });

            const message = `🚨 *NEW EXCLUSIVE LORA ORDER* 🚨\n\n` +
                            `🆔 *Order ID:* \`${orderId}\`\n` +
                            `👤 *Customer Name:* ${name}\n` +
                            `📞 *Phone:* ${phone}\n` +
                            `🏙️ *City:* ${city}\n` +
                            `📍 *Address:* ${address}\n\n` +
                            `📦 *Products List:* ${productsLine}\n\n` +
                            `💰 *Total Price:* *${totalPrice} EGP*\n\n` +
                            `👑 _LORA Perfume Luxury Store Ecosystem_`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const standaloneProductId = currentUrlParams.get("id");
    ...
    onSnapshot(collection(db, "products"), (snapshot) => {
        ...
        const menTarget = document.getElementById("mensProductsDisplayShelfRowGrid");
        const womenTarget = document.getElementById("womensProductsDisplayShelfRowGrid");

        if (menTarget || womenTarget) {
            const menProducts = productsList.filter(p => p.category === "men" || p.category === "unisex");
            const womenProducts = productsList.filter(p => p.category === "women" || p.category === "unisex");
            
            if (menTarget) UI.renderProductCards(menProducts, menTarget);
            if (womenTarget) UI.renderProductCards(womenProducts, womenTarget);
        }

        if (standaloneProductId) {
            const currentProduct = productsList.find(p => p.id === standaloneProductId);
            const detailTarget = document.getElementById("standaloneProductProfileDynamicRenderRoot");
            if (currentProduct && detailTarget) {
                UI.renderStandalonePage(currentProduct, detailTarget);
            }
        }
    });
    ...
    const checkoutForm = document.getElementById("checkoutFormDetails") || document.querySelector("form.checkout-form") || document.getElementById("confirmOrderBtn") || document.getElementById("checkoutProcessingFormStructure");
    if (checkoutForm) {
        const interceptCheckout = async (e) => {
            e.preventDefault();
            try {
                const nameInp = document.getElementById("chkFullName") || document.getElementById("customerFullName");
                const phoneInp = document.getElementById("chkPhoneNumber") || document.getElementById("customerPhoneNumber");
                const addressInp = document.getElementById("chkAddress") || document.getElementById("customerFullAddress");
                const cityInp = document.getElementById("chkCity") || document.getElementById("customerCityName");
                const notesInp = document.getElementById("chkNotes") || document.getElementById("customerOrderNotes");
                ...
                if (!name || !phone || !address || !city) {
                    UI.toast(AppState.lang === 'en' ? "Please fulfill all required fields." : "برجاء ملء الحقول المطلوبة.");
                    return;
                }
                ...
                if (orderedItems.length === 0) {
                    UI.toast(AppState.lang === 'en' ? "Your shopping trunk is currently empty." : "حقيبتك فارغة حالياً.");
                    return;
                }
                ...
                // إرسال البيانات المحدثة والتفصيلية بالكامل إلى التليغرام هنا
                await NotificationEngine.broadcastTelegramAlert(orderRef.id, name, phone, city, address, orderedItems, finalTotal);
                ...
                const premiumModal = document.getElementById("loraPremiumOrderSuccessModalWindow");
                const orderIdSpan = document.getElementById("premiumModalOrderIDDisplayString");
                
                if (orderIdSpan) orderIdSpan.innerText = orderRef.id;
                
                if (premiumModal) {
                    premiumModal.classList.remove("hidden");
                    setTimeout(() => {
                        premiumModal.classList.remove("opacity-0", "scale-95");
                        premiumModal.classList.add("opacity-100", "scale-100");
                    }, 50);
                }

                const contBtn = document.getElementById("premiumModalContinueShoppingActionBtn");
                if (contBtn) {
                ...
                console.error("Order processing ecosystem pipeline safe-caught:", formErr);
